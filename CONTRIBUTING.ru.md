...
Спасибо, что заглянул. HWE — воспроизводимое окружение Wayland + Hyprland на Arch (а с 1.3.0
и на Ubuntu), собранное вокруг двух принципов: **всё автоматизируется** и **цвет живёт в
одном месте**. Держись их — и патч ляжет естественно.
...
```bash
./bin/hwe doctor vm            # проверить хост (libvirtd, группы, KVM, сеть)
./bin/hwe vm up --uncommitted  # быстрый цикл: развернуть рабочее дерево как есть
./bin/hwe vm up                # либо развернуть последний коммит ветки
...
видны сразу. Подробности воркфлоу — в [README](README.ru.md#песочница-и-разработка-vm).
...
На Arch все инструменты выше — пакеты. На **Ubuntu** `ruff` и `actionlint` не запакованы,
поэтому `just lint-py` / `just fmt-py` и шаг actionlint в `lint-ci` там деградируют — поставь
их сам (`pipx install ruff` или `uv tool install ruff`), если хочешь гонять эти гейты локально.
Набор bats — тот, что крутится в CI на обоих дистрибутивах.
...
- **Добавить/изменить тему** → [`themes/README.ru.md`](themes/README.ru.md) (полный гайд по
