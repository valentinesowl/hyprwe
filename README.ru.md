...
> сразу живые. Свои собственные настройки — в `~/.config/hwe/`, см.
> [Твои собственные настройки](#твои-собственные-настройки).
...
трогая: симлинки конфигов, пакеты, личный слой, login-shell, ошибки конфига Hyprland.
Обнаружение и починка используют одни и те же примитивы установки, поэтому проверка не
может разойтись с тем, что реально раскатывает установщик.

---

## Твои собственные настройки

Часть вещей не может прийти из репозитория, потому что дважды одинаковыми они не бывают:
твои мониторы, клавиши, к которым уже привыкли руки, пакеты нужные именно этой машине.
Они живут в `~/.config/hwe/` — **вне клона**, создаются один раз при установке и больше
никогда не переписываются.

| Файл | Что в нём |
|---|---|
| `hypr.conf` | мониторы, ввод, бинды — подключается **последним**, поэтому перекрывает то, что даёт HWE |
| `packages.lst` | дополнительные пакеты этой машины (pacman) |
| `packages-aur.lst` | то же самое, из AUR |
| `waybar.jsonc` | состав твоего бара — примешивается к сгенерированному конфигу при каждой смене темы |

```bash
$EDITOR ~/.config/hwe/hypr.conf    # мониторы сюда, а не в config/hypr/monitors.conf
hyprctl reload                     # SUPER+SHIFT+R
```

Расположение здесь — и есть весь смысл. `config/` раскатан симлинками в репозиторий, так
что правка на месте была бы правкой **трекаемого файла**, а `hwe update` отказывается
тянуть на грязное дерево. Настроить окружение под себя означало бы потерять возможность
его обновлять. А так `git status` остаётся чистым, что бы ты ни менял, pull не может
конфликтовать с твоими настройками, и замена клона не уносит их с собой.

То же правило действует уровнем выше: своя тема кладётся в
`~/.local/share/hwe/themes/<имя>/` и там перекрывает одноимённую поставляемую.

`hwe doctor` показывает, что ты сделал своим, и никогда не называет это расхождением.
Он сообщает только про **отсутствующий** файл — `hwe update` вернёт его на место и никогда
не тронет тот, что уже есть.
...
| `monitors.conf` | значение по умолчанию: каждый выход в предпочтительном режиме (**твои** — в `~/.config/hwe/hypr.conf`) |
...
И самым последним — `~/.config/hwe/hypr.conf`, твой, вне репозитория, перекрывающий всё,
что выше.
...
│   ├── userlayer/          # скелеты ~/.config/hwe — копируются один раз, не перезаписываются
