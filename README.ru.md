...
<p align="center">
  <img src="assets/hero.png" alt="Рабочий стол HWE: пикер тем поверх темы amethyst, htop, Dolphin и сам репозиторий" width="100%">
  <br>
  <em>Свежая установка, которой несколько минут, — тема <code>amethyst</code> красит все компоненты разом: бар, терминал, Qt-файловый менеджер и пикер, выбирающий, что будет дальше.</em>
</p>
...
- **Dev-VM одной командой** — `hwe vm up` поднимает Arch- или Ubuntu-виртуалку
  (`HWE_VM_DISTRO`) через `libvirt`/`virt-install` (видна в virt-manager), провижинит
  через `cloud-init` и разворачивает в ней выбранную локальную ветку git прямо из репозитория.
...
Установка на Arch- или Ubuntu-машину — одна команда: установщик ставит пакеты,
раскатывает конфиги и настраивает вход в систему. Делает это **обратимо**, сохраняя всё, что уже
стоит на машине.

```bash
# 1. Склонировать репозиторий  (на голой системе ещё нет git:
...
- ставит `pkg/core.lst` + `pkg/dev.lst` из репозиториев дистрибутива (на Ubuntu имена
  проходят через `pkg/map/apt.map`); на Arch бутстрапится `paru`, если `pkg/aur.lst`
  непустой — на Ubuntu AUR-пакеты пропускаются, с уведомлением;
...
только на Arch; на Ubuntu он говорит об этом прямо и GPU не трогает. Настройка:
...
- **Обновление системы — по флагу.** Полный апгрейд (`pacman -Su` / `apt-get upgrade`)
  идёт только с `HWE_FULL_UPGRADE=1`; иначе установка работает против текущей БД пакетов
  (на Ubuntu списки пакетов всё равно обновляются — apt требует этого перед любой установкой).
...
- **Без вопросов:** `HWE_ASSUME_YES=1` отвечает «да» на все подтверждения (префлайт установки,
  NVIDIA, синк пакетов, снос VM). Он намеренно **не** покрывает согласие на PPA-композитор
  (`HWE_HYPR_SOURCE=ppa`) — глобальное «да» не должно отдавать ключу третьей стороны такой доступ.
- **Откат:** `hwe uninstall` возвращает прежние конфиги из `*.hwe-bak` и предлагает вернуть
  login-shell (спрашивает); пакеты и сервисы оставляет на месте — заблокировать себя нельзя.
- Мелкие ручки: `NO_COLOR=1` выключает цвет; `HWE_DEFAULT_THEME=<имя>` задаёт тему,
  которую ставит установка (по умолчанию `mocha`).
...
HWE ставится на **Ubuntu 26.04**. Само окружение ради этого не разветвлено: один и тот же
`config/hypr/` разбирается без ошибок обеими версиями Hyprland, а темы рендерятся
одинаково. Отличается упаковка вокруг — и об этом стоит знать заранее.

**Откуда берётся композитор — твоё решение, и HWE не примет его молча.**

| `HWE_HYPR_SOURCE` | Что получаешь | Что выдаёшь |
|---|---|---|
| `repo` *(по умолчанию)* | `hyprland` из Ubuntu, сейчас **0.53.3**, из `universe` | ничего нового — машина уже доверяет своему дистрибутиву |
| `ppa` | актуальный Hyprland (**0.55.4**) из стороннего репозитория | ключ постороннего, ограниченный apt-пином только стеком Hyprland |

```bash
hwe install                        # пакет из Ubuntu
HWE_HYPR_SOURCE=ppa hwe install    # осознанно взять сторонний репозиторий
```

Две вещи, о которых нужно сказать прямо. `universe` поддерживается сообществом: на него
**не распространяется пятилетнее обязательство по безопасности** — оно есть только у
`main`. И подключение стороннего репозитория обычным способом даёт его ключу право
поставить на машину *любой* пакет и навсегда; HWE такой репозиторий пинует так, чтобы он
мог поставлять стек композитора и ничего больше. Установка `HWE_HYPR_SOURCE=ppa` **и есть**
согласие — поэтому `HWE_ASSUME_YES` его не покрывает: общее «да» на вопросы не должно
оказываться тем, чем выдаётся доверие постороннему. Сборка композитора из исходников не
предлагается.

Ещё два отличия, чтобы не стали сюрпризом:

- **Ubuntu включает то, что установила.** Пакеты Debian-семейства запускают и включают
  свои сервисы прямо при установке, в том числе пользовательские systemd-юниты для
  программ сессии. HWE запускает их из `config/hypr/autostart.conf`, поэтому пакетные
  юниты снимаются — иначе получишь два бара и два демона простоя.
- **Части инструментов в архиве нет.** `satty` (аннотация скриншотов — два бинда
  деградируют до обычного снимка), `hyprsunset` (ночной свет — луна в баре прячется, а
  `hwe sunset` говорит почему; в PPA он есть, так что на том пути его возвращает один
  `apt install hyprsunset`), `ruff` и `actionlint`. Для `ruff` у `just lint-py`
  и `just fmt-py` есть запасная дверь — запуск через uv: один раз `pipx install uv`
  (pipx упакован; самого uv в архиве тоже нет) — и они работают. У `actionlint`
  (`just lint-ci`) такой двери нет — он остаётся отсутствующим. Все такие различия
  записаны в `pkg/map/apt.map`, вместе с причиной.
...
hwe doctor --report   # та же проверка одним готовым для issue блоком
```
...
стек screen sharing (pipewire, wireplumber, порталы).
...
| `packages.lst` | дополнительные пакеты этой машины (из репозиториев её дистрибутива) |
| `packages-aur.lst` | то же самое, из AUR (Arch; на Ubuntu пропускается) |
| `waybar.jsonc` | состав твоего бара — примешивается к сгенерированному конфигу при каждой смене темы |
| `hyprsunset.conf` | твоё расписание ночного света (когда и насколько теплее) — `hwe sunset` включает и выключает |
| `kanshi.conf` | твои профили мониторов — воткнул док, раскладка перестроилась; выдернул — вернулась |
| `themes/` | заметки самого HWE о темах — какая применена, какие обои у каждой. Пишет их `hwe`, так что замена клона ничего не забывает |
...
Для ноутбука, который встречается с доком, мониторам лучше в `kanshi.conf`: профиль
применяется в момент, когда подключены все его выходы, — воткнул, и раскладка перестроилась
сама; выдернул — вернулась. Но у выхода один хозяин: дисплею из kanshi-профиля не место в
`monitor =`-строке `hypr.conf`, иначе каждый reload возобновляет их спор.
...
Он сообщает только про **отсутствующий** файл — `hwe update` вернёт его на место. Файл,
который ты правил, никогда не переписывается; а нетронутый — тихо следует за поставляемыми
дефолтами (HWE помнит каждый дефолт, который когда-либо отгружал, и потому отличает одно от
другого). Захочется вернуть заводское: `hwe update --reset-defaults` восстановит дефолты,
сохранив твои версии рядом как `*.hwe-bak`.
...
| `hwe sunset <toggle\|on\|off\|status>` | ночной свет (hyprsunset; луна в правом краю бара) |
...
| `hwe doctor [host\|vm\|--report]` | проверить эту машину (host, по умолчанию) или пред-условия для VM; `--report` = готовый блок для issue |
...
`hwe vm up` поднимает **одноразовую** Arch- или Ubuntu-виртуалку (`HWE_VM_DISTRO`, видна в
virt-manager) и разворачивает в ней **твою локальную ветку git** прямо из репозитория. Внутри
отрабатывает тот же `hwe install`, что и на железе, так что это честная репетиция установки.
...
1. `hwe vm up <branch>` качает cloud-образ гостя в `~/.cache/hwe/` (один раз, несколько
   сотен МБ) и **сверяет его GPG-подпись** — с закреплённым fingerprint'ом arch-boxes для
   Arch, или с ключом подписи Ubuntu cloud-образов (подписанный `SHA256SUMS`) для Ubuntu.
   Образ, который HWE не может аутентифицировать, отклоняется, а не используется.
...
|---|---|---|
| `HWE_VM_DISTRO` | `arch` | дистрибутив гостя: `arch` или `ubuntu` |
| `HWE_VM_UBUNTU_RELEASE` | `26.04` | релиз Ubuntu для загрузки (при `HWE_VM_DISTRO=ubuntu`) |
| `HWE_HYPR_SOURCE` | `repo` | откуда в госте Hyprland: `repo` или `ppa` |
| `HWE_VM_NAME` | по дистрибутиву | имя домена libvirt (`hwe-dev`, `hwe-dev-ubuntu`) |
...
| `HWE_IMAGE_URL` | по дистрибутиву | базовый образ (подписанный; неподтверждённый отвергается) |
...
│   ├── sunset.sh           # hwe sunset — ночной свет (hyprsunset)
...
│   └── *.asc               # пиннинг ключей подписи cloud-образов (arch-boxes, Ubuntu)
...
│   ├── kanshi/             # профили мониторов на hotplug (сами профили: ~/.config/hwe)
...
`v1.4.2` — релиз для каждого дня, плюс doctor, отвечающий на вопросы, с которыми люди
реально приходят: заработает ли screen sharing (цепочка pipewire → порталы — проверяется)
и «что это за машина» (`hwe doctor --report` — готовый блок для вставки в issue).
Ночной свет по твоему расписанию (`hwe sunset`, луна в
