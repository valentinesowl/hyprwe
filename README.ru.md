...
- **Части инструментов в архиве нет.** `satty` (аннотация скриншотов — два бинда
  деградируют до обычного снимка), `ruff` и `actionlint`: последние два — гейты
  разработки, так что `just lint-py` и `just fmt-py` на этом дистрибутиве требуют их из
  `pip`/`uv`. Все такие различия записаны в `pkg/map/apt.map`, вместе с причиной.
